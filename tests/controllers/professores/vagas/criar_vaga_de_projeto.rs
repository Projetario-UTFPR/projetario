...
use inertia_rust::test::InertiaTestRequest;
...
use crate::common::utils::queries::salvar_usuario;
...
async fn ninguem_deveria_poder_criar_vagas_invalidas(
    ...
    dbg!(&body);
    dbg!(resposta.response().body());
    assert_eq!(StatusCode::FOUND, resposta.status());

    let count: i64 = sqlx::query_scalar("SELECT COUNT(id) FROM projeto")
...
async fn um_professor_deveria_poder_criar_vagas_para_seus_projetos() { todo!() }

async fn um_aluno_nao_deveria_poder_criar_vagas() { todo!() }
