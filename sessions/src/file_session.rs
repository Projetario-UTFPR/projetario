...
impl Default for FileSessionStore<'_> {
    fn default() -> Self {
        Self {
            sessions_directory: AppConfig::get().sessions_dir,
        }
    }
}
...
impl SessionStore for FileSessionStore<'_> {
    async fn load(&self, session_key: &SessionKey) -> Result<Option<SessionState>, LoadError> {
        match File::open(self.get_session_path(session_key.as_ref())).await {
            ...
            Ok(mut session) => {
                ...
                if let Some(expiration_date) = session.get(AppConfig::get().sessions_exp_key) {
                    let has_expired =
    ...
    async fn update(
        ...
        let has_expired = !(old_session.is_ok_and(|file| {
            file.get(AppConfig::get().sessions_exp_key)
...
impl FileSessionStore<'_> {
    ...
    fn set_expiration_date(
        ...
        session_state.insert(AppConfig::get().sessions_exp_key.into(), expiration_date);
        session_state
    }
...
pub async fn clean_expired_sessions() -> io::Result<()> {
    ...
    inner_clean_expired_sessions(AppConfig::get().sessions_dir).await
}
...
async fn file_should_be_cleaned(file: &DirEntry) -> bool {
    ...
    match session.get(AppConfig::get().sessions_exp_key) {
        None => true,
